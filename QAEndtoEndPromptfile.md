...
This prompt guides you through a complete **7-step QA workflow using MCP servers and AI agents** to go from **user story → committed automated test scripts**.
...
```
https://github.com/ravikanathe/AgentE2EQAWorkflow-Playwright.git
```
...
Use Playwright browser tools to manually execute each test scenario.
...
Review the test plan and exploratory testing results.
Use the **playwright-test-generator agent** to create Playwright JavaScript automation scripts leveraging the selectors and insights discovered during manual testing.
...
Run all automation scripts from:

```
tests/saucedemo-checkout/
```

Use the **playwright-test-healer agent** to identify and auto-heal failing tests until all are stable and passing.
...
Include PASS/FAIL status, healing summary, defects log, and test coverage analysis.
